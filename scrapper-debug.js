...
async function scrape(url){
    ...
    for (let j = 0; j < urlsList.length; j++) {
        for (let k = 0; k < urlsList[j].length; k++) {
            ...
            // making an array with all class details.
            const elems = await page.evaluate(() => {
                ...
                // This list contains the JSON object for each class.
                const elemList = [];
                // Iterating over the DOM Elements
                pageElements.forEach((word)=>{
                    // This selector is for the table section of each page.
                    var div = word.querySelectorAll('div > table > tbody > tr');
                    ...
                    div.forEach((x)=>{
                        ...
                        var values = x.textContent.replace(/\s+/g,' ').trim().match(/([\w+]+)/g);
                        // If we encounter a string LECTURE/LAB.
                        if (values[1] !== 'LAB' && values[1] !== 'LECTURE' && values[1] !== 'SEMINAR') {
                            elemList.push(
                                {
                                    "courseName": document.querySelector('h1').innerText.substring(0, document.querySelector('h1').innerText.indexOf('-')-1),
                                    // "courseNumber": document.querySelector('h1').innerText.split(' ')[1],
                                    "courseTerm": x.parentElement.parentElement.parentElement.parentElement.querySelector('h2').textContent,
...
// scrape('https://apps.ualberta.ca/catalogue/course/');
...
async function scrapeTest(url){
    ...
    for (let l = 0; l < lec_json.length; l++) {
        if (lec_json[l] !== null) {
            console.log(lec_json[l]);
        };
    }
    await browser.close();
};
scrapeTest('https://apps.ualberta.ca/catalogue/course/cmput/174');
